...
SaberStatus SaberPooling<AMD, OpDtype>::create(
    ...
    kernelInfo.comp_options =
            ...
            + std::string(" -DMLO_POOLING_GROUP_SZ0=8") + std::string(" -DMLO_POOLING_GROUP_SZ1=8")

            + std::string(" -DMLO_POOLING_BOT_BATCH_STRIDE=")
            + std::to_string(inputs[0]->width() * inputs[0]->height() * outputs[0]->channel())
            + std::string(" -DMLO_POOLING_BOT_CHANNEL_STRIDE=")
            + std::to_string(inputs[0]->width() * inputs[0]->height())
            + std::string(" -DMLO_POOLING_BOT_STRIDE=") + std::to_string(inputs[0]->width())
            + std::string(" -DMLO_POOLING_TOP_BATCH_STRIDE=")
            + std::to_string(outputs[0]->width() * outputs[0]->height() * outputs[0]->channel())
            + std::string(" -DMLO_POOLING_TOP_CHANNEL_STRIDE=")
            + std::to_string(outputs[0]->width() * outputs[0]->height())
            ...
            + std::string(" -DCU_NUM=64") + std::string(" -DMIOPEN_USE_FP32=1")
