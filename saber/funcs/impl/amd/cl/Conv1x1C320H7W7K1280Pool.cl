...
#define OFFSET ((grid_x % COUNTER_STRIDE) * 16 % STRIDE_WEI)
...
void reduce(__local float* buffer, int tid) {
    ...
    if (tid < 256 && (tid & 15) == 0) {
        buffer[tid + (tid >> 5)] += buffer[tid + 256 + ((tid + 256) >> 5)]
                                    + buffer[tid + 512 + ((tid + 512) >> 5)]
                                    + buffer[tid + 768 + ((tid + 768) >> 5)];
    }
    barrier(CLK_LOCAL_MEM_FENCE);
#endif
}
...
__attribute__((reqd_work_group_size(1024, 1, 1))) __kernel void conv1x1_act_pool(
    ...
    __global const float* pWei =
            (__global const float*)(wei + (WEI_ROW_START * STRIDE_WEI + WEI_WI_COL_START)); //
    ...
    uint offset = 0; //((grid_x & 63) << 4) % STRIDE_WEI;
    ...
    for (uchar k = 0; k < ITER; k++, offset = (offset + PER_ITER_STRIDE) % STRIDE_WEI)
    {
        ...
        {
            shared_wei[WEI_WI_COL_START * 17 + (LDS_WEI_ROW_START + i)] = pWei[(LDS_WEI_ROW_START + i) * STRIDE_WEI + offset];
        }
        for (uchar i = 0; i < LDS_IN_READ_ITER; i++)
        {
            shared_in[(LDS_IN_ROW_START + i) * LDS_IN_STRIDE + IN_WI_COL_START + (IN_WI_COL_START >> 5)] = (IN_WI_COL_START < STRIDE_IN ? pIn[(LDS_IN_ROW_START + i + offset) * STRIDE_IN] : 0.0f); //pIn[(LDS_IN_ROW_START + i + offset) * STRIDE_IN];
        }
        barrier(CLK_LOCAL_MEM_FENCE);

        for (uchar j = 0; j < PER_ITER_STRIDE; j++)
        {
            sum += pShared_wei[j * 17 + OUT_WI_ROW_START] * pShared_in[j * LDS_IN_STRIDE + IN_WI_COL_START + (IN_WI_COL_START >> 5)];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
#elif 0
    for (uchar k = 0; k < ITER; k++, offset = (offset + PER_ITER_STRIDE) % STRIDE_WEI) {
        for (uchar i = 0; i < LDS_WEI_READ_ITER; i++) {
            shared_wei
                    [WEI_WI_COL_START + (WEI_WI_COL_START >> 5)
                     + (LDS_WEI_ROW_START + i) * LDS_WEI_STRIDE] =
                            pWei[(LDS_WEI_ROW_START + i) * STRIDE_WEI + offset];
        }
        for (uchar i = 0; i < LDS_IN_READ_ITER; i++) {
            shared_in
                    [(LDS_IN_ROW_START + i) + ((LDS_IN_ROW_START + i) >> 5)
                     + IN_WI_COL_START * LDS_WEI_STRIDE] =
    ...
    for (uchar k = 0; k < ITER; k++, offset = (offset + PER_ITER_STRIDE) % STRIDE_WEI) {
        for (uchar i = 0; i < LDS_WEI_READ_ITER; i++) {
            if (LDS_WEI_ROW_START + i * 8 < LDS_WEI_ROW)
                shared_wei
                        [WEI_WI_COL_START + (WEI_WI_COL_START >> 5)
                         + (LDS_WEI_ROW_START + i * 8) * LDS_WEI_STRIDE] =
                                (LDS_WEI_WI < LDS_WEI_COL_REAL
                                         ? pWei[(LDS_WEI_ROW_START + i * 8) * STRIDE_WEI + offset]
                                         : 0.0f);
        }
        for (uchar i = 0; i < LDS_IN_READ_ITER; i++) {
            shared_in
                    [(LDS_IN_ROW_START + i) * LDS_IN_STRIDE + IN_WI_COL_START
                     + (IN_WI_COL_START >> 5)] =
                            (IN_WI_COL_START < STRIDE_IN
                                     ? pIn[(LDS_IN_ROW_START + i + offset) * STRIDE_IN]
                                     : 0.0f); // pIn[(LDS_IN_ROW_START + i + offset) * STRIDE_IN];
        ...
        for (uchar j = 0; j < PER_ITER_STRIDE; j++) {
            sum += (lid_x < OUT_PER_WG
                            ? pShared_wei[j + (j >> 5) + (COMPUTE_WEI_WI_INDEX * LDS_WEI_STRIDE)]
                                      * pShared_in
                                                [j * LDS_IN_STRIDE + COMPUTE_IN_WI_INDEX
                                                 + (COMPUTE_IN_WI_INDEX >> 5)]
                            : 0.0f);
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    ...
    for (uchar k = 0; k < ITER; k++, offset = (offset + PER_ITER_STRIDE) % STRIDE_WEI) {
        ...
        for (uchar j = 0; j < 32; j++) {
            for (uchar l = 0; l < 4; l++) {
                sum += pShared_wei
                               [(lid_x >> 8 << 5) + j + (((lid_x >> 8 << 5) + j) >> 5)
                                + ((((lid_x & 255) >> 4)) * LDS_WEI_STRIDE)]
                       * pShared_in
                                 [(lid_x >> 8 << 5) + j + (((lid_x >> 8 << 5) + j) >> 5)
                                  + (((lid_x & 15) << 2) + l) * LDS_IN_STRIDE];
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
    ...
    for (uchar k = 0; k < ITER; k++, offset = (offset + PER_ITER_STRIDE) % STRIDE_WEI) {
        ...
        for (uchar i = 0; i < LDS_IN_READ_ITER; i++) {
#if 0		
            shared_in[(LDS_IN_ROW_START + i) * LDS_IN_STRIDE + IN_WI_COL_START + (IN_WI_COL_START >> 5)] = pIn[(LDS_IN_ROW_START + i + offset) * STRIDE_IN];
#else
            shared_in
                    [(LDS_IN_ROW_START + i) * LDS_IN_STRIDE + IN_WI_COL_START
                     + (IN_WI_COL_START >> 5)] =
        ...
        for (uchar j = 0; j < 32; j++) {
            for (uchar l = 0; l < 4; l++) {
                sum += pShared_wei[((lid_x >> 8 << 5) + j) * 17 + ((lid_x & 255) >> 4)]
                       * pShared_in
                                 [((lid_x >> 8 << 5) + j) * LDS_IN_STRIDE + ((lid_x & 15) << 2) + l
                                  + ((((lid_x & 15) << 2) + l) >> 5)];
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
...
#ifdef DIR
#ifdef BIAS
    sum += lid_x < OUT_PER_WG ? pBias[0] : 0.0f;
#endif
    sum *= (sum > 0.0f ? 1.0f : slope);
    shared_result
            [(COMPUTE_WEI_WI_INDEX * 64 + COMPUTE_IN_WI_INDEX)
             + ((COMPUTE_WEI_WI_INDEX * 64 + COMPUTE_IN_WI_INDEX) >> 5)] = sum;
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    if (lid_x < OUT_PER_WG_REAL) {
        pOut[0] = shared_result[(lid_x << 6) + (lid_x << 6 >> 5)] / 49.0f;
    }
#else
#ifdef BIAS
