...
SaberStatus SaberLayerNorm<AMD, OpDtype>::create(
    ...
    if (!_kernel_ptr_reduce_mean.get()->isInit()) {
        ALOGE("Failed to load _kernel_ptr_reduce_mean");
    ...
    if (!_kernel_ptr_reduce_std.get()->isInit()) {
        ALOGE("Failed to load _kernel_ptr_reduce_std");
        return SaberInvalidValue;
    }
    ...
    if (!_kernel_ptr_normalize_with_scale_bias.get()->isInit()) {
        ALOGE("Failed to load _kernel_ptr_normalize_with_scale_bias");
        return SaberInvalidValue;
    }
...
SaberStatus SaberLayerNorm<AMD, OpDtype>::dispatch(
    ...
    if (!err) {
        ALOGE("Fail to set _kernel_ptr_reduce_mean->SetKernelArgs");
        return SaberInvalidValue;
    }
    ...
    err = kernel->SetKernelArgs(
            ...
            (int)flag_bias);
    if (!err) {
        ALOGE("Fail to set _kernel_ptr_normalize_with_scale_bias->SetKernelArgs");
        return SaberInvalidValue;
    }
