...
SaberStatus SaberFc<AMD, OpDtype>::create(
    ...
    const int gwk[5][6] = {{VGG16_FC6_NT_GLOBAL_WORK_SIZE_BS1,
    ...
    const int lwk[5][6] = {{VGG16_FC6_NT_LOCAL_WORK_SIZE_BS1,
    ...
    const std::string kfn[5][6] = {{VGG16_FC6_NT_KERNEL_FILE_NAME_BS1,
...
SaberStatus SaberFc<AMD, OpDtype>::dispatch(
    ...
    ALOGD("dispatch\n");
    ALOGD("num_output= " << param.num_output << " axis=" << param.axis);
    ALOGD("in num= " << inputs[0]->num() << " channel= " << inputs[0]->channel()
                     << " height= " << inputs[0]->height() << " width= " << inputs[0]->width());
    ALOGD("weight num= " << param.weights->num() << " channel= " << param.weights->channel()
