...
SaberStatus SaberScale<AMD, OpDtype>::create(
    ...
    if (!_kernel_Scale_singleBias.get()->isInit()) {
        ALOGE("Failed to load _kernel_Scale_singleBias ");
    ...
    if (!_kernel_Scale_multiBias.get()->isInit()) {
        ALOGE("Failed to load program _kernel_Scale_multiBias");
        return SaberInvalidValue;
    }
...
SaberStatus SaberScale<AMD, OpDtype>::dispatch(
    ...
    if (_scale_dim > 1 || inputs.size() > 1) //_kernel_Scale_multiBias
    {
        ...
        err               = kernel->SetKernelArgs(
                ...
                (int)bias_flag);
        if (!err) {
            ALOGE("Fail to set _kernel_Scale_multiBias->SetKernelArgs");
            return SaberInvalidValue;
        }
        ...
        err               = kernel->SetKernelArgs(
                ...
                (int)count);
        if (!err) {
            ALOGE("Fail to s _kernel_Scale_singleBias->SetKernelArgs");
            return SaberInvalidValue;
        }
