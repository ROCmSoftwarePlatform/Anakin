...
*/
#include "include/saber_deformable_conv.h"
...
SaberStatus SaberDeformableConv2D<AMD, OpDtype>::init(
...
template <DataType OpDtype>
SaberStatus SaberDeformableConv2D<AMD, OpDtype>::create(
        const std::vector<Tensor<AMD>*>& inputs,
        std::vector<Tensor<AMD>*>& outputs,
        ConvParam<AMD>& param,
        Context<AMD>& ctx) {
...
SaberStatus SaberDeformableConv2D<AMD, OpDtype>::dispatch(
        const std::vector<Tensor<AMD>*>& inputs,
        std::vector<Tensor<AMD>*>& outputs,
        ConvParam<AMD>& param) {
    bool err;
...
DEFINE_OP_TEMPLATE(SaberDeformableConv2D, ConvParam, AMD, AK_INT8);
DEFINE_OP_TEMPLATE(SaberDeformableConv2D, ConvParam, AMD, AK_HALF);
} // namespace saber
