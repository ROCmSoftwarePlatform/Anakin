...
void fc_cpu_base(
    ...
    Tensor<TargetType_H> bias_h;
    bool bias_enable = param.bias && param.bias->valid_size() > 0;
    if (bias_enable) {
        bias_h.re_alloc(param.bias->valid_shape());
        bias_h.copy_from(*param.bias);
        bias = (const dtype*)bias_h.data();
    }
    ...
    for (int i = 0; i < out_rows; i++) {
        for (int j = 0; j < out_cols; j++) {
            ...
            data_out[index_out] = bias_enable ? bias[j] : 0;
...
TEST(TestSaberFunc, test_op_fc) {
    ...
    // Shape shape_weight({})
    // Specific case
    // InnerProduct YOLO-FC26: [1,4096]*[4096,1470]
    int w_in    = 1;
    int h_in    = 1;
    int ch_in   = 4096;
    int out_num = 1470;
    int num_in  = 1;
    Shape shape({num_in, ch_in, h_in, w_in});
    Shape shape_hw({1, 1, out_num, ch_in});
    weights_h.re_alloc(shape_hw, AK_FLOAT);
    weights_d.re_alloc(shape_hw, AK_FLOAT);
    fill_tensor_rand(weights_h, 0.1, 1.5);
    weights_d.copy_from(weights_h);
    Shape shape_b({1, out_num, 1, 1});
    Tensor<AMD> bias;
    bias.re_alloc(shape_b, AK_FLOAT);
    fill_tensor_rand(bias, 0.0, 1.0);
    FcParam<AMD> param(&weights_d, &bias, out_num);
    testbase.set_param(param);
    testbase.set_rand_limit(1, 12);
    testbase.set_input_shape(shape);
